...
@section('title', 'Edit Laporan Kerjaan')
@section('breadcrumb')
...
@section('content')
    <div class="bg-white rounded-xl border border-slate-200 p-6 max-w-3xl mx-auto">
        <h2 class="text-lg font-semibold text-slate-900 mb-6">Edit Laporan Kerjaan</h2>
        ...
        <form method="POST" action="{{ route('work-orders.update', $workOrder) }}" class="space-y-5">
            ...
            <div class="space-y-1.5">
                ...
                <select name="equipment_tag" id="equipment_tag" required
                    ...
                    @foreach($equipmentList as $eq)
                        <option value="{{ $eq->equipment_tag }}" {{ old('equipment_tag', $workOrder->equipment_tag) === $eq->equipment_tag ? 'selected' : '' }}>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="space-y-1.5">
                    ...
                    <input type="date" name="tanggal_kejadian" value="{{ old('tanggal_kejadian', $workOrder->tanggal_kejadian->format('Y-m-d')) }}" required
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="date" name="tanggal_eksekusi_selesai" value="{{ old('tanggal_eksekusi_selesai', $workOrder->tanggal_eksekusi_selesai->format('Y-m-d')) }}" required
                           class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500">
            ...
            <div class="space-y-1.5">
                ...
                <div class="grid grid-cols-3 gap-3">
                    <label class="flex items-center p-3 border border-slate-200 rounded-lg cursor-pointer hover:bg-slate-50 has-[:checked]:border-teal-500 has-[:checked]:bg-teal-50 transition-colors">
                        <input type="radio" name="jenis_pekerjaan" value="CM" {{ old('jenis_pekerjaan', $workOrder->jenis_pekerjaan) === 'CM' ? 'checked' : '' }}
                    ...
                    <label class="flex items-center p-3 border border-slate-200 rounded-lg cursor-pointer hover:bg-slate-50 has-[:checked]:border-amber-500 has-[:checked]:bg-amber-50 transition-colors">
                        <input type="radio" name="jenis_pekerjaan" value="dCM" {{ old('jenis_pekerjaan', $workOrder->jenis_pekerjaan) === 'dCM' ? 'checked' : '' }}
                    ...
                    <label class="flex items-center p-3 border border-slate-200 rounded-lg cursor-pointer hover:bg-slate-50 has-[:checked]:border-blue-500 has-[:checked]:bg-blue-50 transition-colors">
                        <input type="radio" name="jenis_pekerjaan" value="PM" {{ old('jenis_pekerjaan', $workOrder->jenis_pekerjaan) === 'PM' ? 'checked' : '' }}
                ...
                <select name="linked_finding_id" id="linked_finding_id"
                    ...
                    @if($findings->isNotEmpty())
                        @foreach($findings as $f)
                            <option value="{{ $f->id }}" {{ old('linked_finding_id', $workOrder->linked_finding_id) == $f->id ? 'selected' : '' }}>
                                {{ $f->kode_finding ?? '#' . $f->id }} — {{ $f->kategori }} ({{ $f->tanggal_temuan }})
                            </option>
                        @endforeach
                    @endif
                </select>
            ...
            <div id="sesuaiRencanaField" class="space-y-1.5 {{ old('jenis_pekerjaan', $workOrder->jenis_pekerjaan) === 'dCM' ? '' : 'hidden' }}">
                <label class="block text-sm font-medium text-slate-700">
                    Sesuai Rencana?
                ...
                <div class="flex gap-4">
                    <label class="inline-flex items-center">
                        <input type="radio" name="sesuai_rencana" value="ya" {{ old('sesuai_rencana', $workOrder->sesuai_rencana) === 'ya' ? 'checked' : '' }}
                               class="mr-2 text-green-600 focus:ring-green-500">
                        <span class="text-sm text-slate-700">Ya</span>
                    </label>
                    <label class="inline-flex items-center">
                        <input type="radio" name="sesuai_rencana" value="tidak" {{ old('sesuai_rencana', $workOrder->sesuai_rencana) === 'tidak' ? 'checked' : '' }}
            ...
            <div class="space-y-1.5">
                ...
                <textarea name="deskripsi" rows="3"
                          class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500">{{ old('deskripsi', $workOrder->deskripsi) }}</textarea>
                @error('deskripsi')
                    <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="space-y-1.5">
                ...
                <input type="text" name="root_cause" value="{{ old('root_cause', $workOrder->root_cause) }}" maxlength="255"
                       class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500"
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (eqSelect) {
            eqSelect.addEventListener('change', function() {
                var tag = this.value;
                var currentVal = findingSelect.value;
