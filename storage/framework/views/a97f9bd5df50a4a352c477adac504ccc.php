<?php $__env->startSection("page-title", $equipment->equipment_tag . " — Detail Equipment"); ?>
<?php $__env->startSection("page-sub", "Informasi detail & riwayat pembacaan equipment"); ?>
...
<div class="bg-white rounded-xl border border-slate-200 p-6 mb-6">
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div class="min-w-0">
            <div class="flex items-center gap-3 flex-wrap">
                <h2 class="text-2xl font-bold font-mono" style="color: #0E9E8E;"><?php echo e($equipment->equipment_tag); ?></h2>
                <?php
                ...
                ?>
                <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium border <?php echo e($stColor); ?>">
                    <span class="w-2 h-2 rounded-full mr-1.5 <?php echo e($dotColor); ?>"></span>
                    <?php echo e($stLabel); ?>

                </span>
            </div>
            <p class="text-sm text-slate-500 mt-1">
                <?php echo e($equipment->pt_location); ?>

                <?php if($equipment->plant): ?>
                    &middot; <?php echo e($equipment->plant); ?>

                <?php endif; ?>
            </p>
            <p class="text-sm text-slate-700 mt-1">
                <?php if($equipment->asset): ?>
                    <?php echo e($equipment->asset->description); ?>

                <?php else: ?>
                    <span class="text-amber-600">Asset belum terdaftar di Asset Management.</span>
                    <a href="<?php echo e(route('assets.create')); ?>?equipment_no=<?php echo e(urlencode($equipment->equipment_tag)); ?>"
        ...
        <div class="shrink-0 flex items-center gap-2">
            <a href="<?php echo e(route('cm.report-analysis')); ?>"
            ...
            <a href="<?php echo e(route('cm.equipment-status')); ?>" class="inline-flex items-center gap-1.5 px-4 py-2 border border-slate-300 hover:bg-slate-50 text-slate-700 text-sm font-medium rounded-lg transition-colors">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/></svg>
                Equipment Status
            </a>
        </div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mt-5 pt-5 border-t border-slate-100">
        <div>
            ...
            <p class="text-sm font-medium text-slate-800 mt-0.5"><?php echo e($equipment->tipe_lubrikasi ?? "—"); ?></p>
        </div>
        <div>
            <p class="text-xs text-slate-400 uppercase tracking-wide">PT / Plant</p>
            <p class="text-sm font-medium text-slate-800 mt-0.5"><?php echo e($equipment->pt_location); ?> / <?php echo e($equipment->plant ?? "—"); ?></p>
        </div>
        <div>
            <p class="text-xs text-slate-400 uppercase tracking-wide">Total Readings</p>
            <p class="text-sm font-medium text-slate-800 mt-0.5"><?php echo e($equipment->readings()->count()); ?></p>
        </div>
        <div>
            <p class="text-xs text-slate-400 uppercase tracking-wide">Total Findings</p>
            <p class="text-sm font-medium text-slate-800 mt-0.5"><?php echo e($totalFindingsOpen + $totalFindingsClosed); ?> <span class="text-xs text-slate-400 font-normal">(<?php echo e($totalFindingsOpen); ?> open / <?php echo e($totalFindingsClosed); ?> closed)</span></p>
        </div>
    </div>
</div>
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
    <div class="bg-white rounded-xl border border-slate-200 p-5 relative">
        ...
        <div class="space-y-3">
            <div>
                ...
                <p class="text-sm text-slate-800 font-medium"><?php echo e($equipment->asset?->company?->name ?? $equipment->asset?->company?->code ?? "—"); ?></p>
            </div>
            <div>
                <p class="text-xs text-slate-400">Equipment No (SAP)</p>
                <p class="text-sm font-mono text-slate-800"><?php echo e($equipment->asset?->equipment_no ?? "—"); ?></p>
            </div>
            <div>
                <p class="text-xs text-slate-400">Tech Ident No (SAP)</p>
                <p class="text-sm font-mono text-slate-800"><?php echo e($equipment->asset?->tech_ident_no ?? "—"); ?></p>
            </div>
            <div>
                <p class="text-xs text-slate-400">Description</p>
                <p class="text-sm text-slate-800"><?php echo e($equipment->asset?->description ?? "—"); ?></p>
            </div>
            <div>
                <p class="text-xs text-slate-400">Manufacturer</p>
                <p class="text-sm text-slate-800"><?php echo e($equipment->asset?->manufacturer ?? "—"); ?></p>
            </div>
            <div>
                <p class="text-xs text-slate-400">Model Number</p>
                <p class="text-sm text-slate-800"><?php echo e($equipment->asset?->model_number ?? "—"); ?></p>
            </div>
            <div>
                <p class="text-xs text-slate-400">Construct Year</p>
                <p class="text-sm text-slate-800"><?php echo e($equipment->asset?->construct_year ?? "—"); ?></p>
            </div>
        </div>
        <?php if(!$equipment->asset): ?>
    ...
    <div class="lg:col-span-2 grid grid-cols-2 sm:grid-cols-3 gap-4">
        <div class="bg-white rounded-xl border border-slate-200 p-4 flex flex-col justify-between">
            ...
            <p class="text-2xl font-bold text-slate-900 mt-1"><?php echo e($equipment->readings()->count()); ?></p>
        </div>
        <div class="bg-white rounded-xl border border-slate-200 p-4 flex flex-col justify-between">
            <p class="text-xs text-slate-400 uppercase tracking-wide font-medium">Total Findings</p>
            <div class="mt-1">
                <p class="text-2xl font-bold text-slate-900"><?php echo e($totalFindingsOpen + $totalFindingsClosed); ?></p>
                <p class="text-xs mt-0.5"><span class="text-amber-600 font-medium"><?php echo e($totalFindingsOpen); ?> open</span><span class="text-slate-300 mx-1">/</span><span class="text-green-600 font-medium"><?php echo e($totalFindingsClosed); ?> closed</span></p>
            </div>
        </div>
        <div class="bg-white rounded-xl border border-slate-200 p-4 flex flex-col justify-between">
            <p class="text-xs text-slate-400 uppercase tracking-wide font-medium">Last Reading</p>
            <div class="mt-1">
                <p class="text-sm font-semibold text-slate-800"><?php echo e($lastReading ? \Carbon\Carbon::parse($lastReading->tanggal)->format("d M Y") : "—"); ?></p>
                <?php if($lastReading): ?>
                    <span class="inline-flex items-center gap-1 text-xs font-medium mt-0.5 <?php echo e($lastReading->kondisi === "good" ? "text-green-600" : ($lastReading->kondisi === "alarm" ? "text-amber-600" : ($lastReading->kondisi === "danger" ? "text-red-600" : "text-purple-600"))); ?>">
                        <span class="w-1.5 h-1.5 rounded-full <?php echo e($lastReading->kondisi === "good" ? "bg-green-500" : ($lastReading->kondisi === "alarm" ? "bg-amber-500" : ($lastReading->kondisi === "danger" ? "bg-red-500" : "bg-purple-500"))); ?>"></span>
                        <?php echo e(ucfirst(str_replace("_", " ", $lastReading->kondisi))); ?>
        ...
        <div class="bg-white rounded-xl border border-slate-200 p-4 flex flex-col justify-between">
            ...
            <p class="text-2xl font-bold <?php echo e($vibMax !== "-" && (float) $vibMax > 10 ? "text-red-600" : "text-slate-900"); ?> mt-1"><?php echo e($vibMax); ?> <span class="text-sm font-normal text-slate-400">mm/s</span></p>
        </div>
        <div class="bg-white rounded-xl border border-slate-200 p-4 flex flex-col justify-between">
            <p class="text-xs text-slate-400 uppercase tracking-wide font-medium">Temp Maks</p>
            <p class="text-2xl font-bold <?php echo e($tempMax !== "-" && (float) $tempMax > 80 ? "text-red-600" : "text-slate-900"); ?> mt-1"><?php echo e($tempMax); ?> <span class="text-sm font-normal text-slate-400">&deg;C</span></p>
        </div>
        <div class="bg-white rounded-xl border border-slate-200 p-4 flex flex-col justify-between">
            <p class="text-xs text-slate-400 uppercase tracking-wide font-medium">Ampere</p>
            <p class="text-2xl font-bold text-slate-900 mt-1"><?php echo e($lastReading?->ampere ? number_format($lastReading->ampere, 1) : "—"); ?> <span class="text-sm font-normal text-slate-400">A</span></p>
        </div>
    </div>
</div>
<div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
    <div class="bg-white rounded-xl border border-slate-200 p-4 flex flex-col justify-between">
        ...
        <p class="text-2xl font-bold mt-1" style="color: #0E9E8E;"><?php if($mtbfData["mtbf"] !== "-"): ?><?php echo e(number_format($mtbfData["mtbf"], 1)); ?> <span class="text-sm font-normal text-slate-400">hari</span><?php else: ?>-<?php endif; ?></p>
    </div>
    <div class="bg-white rounded-xl border border-slate-200 p-4 flex flex-col justify-between">
        <p class="text-xs text-slate-400 uppercase tracking-wide font-medium">Total Failures</p>
        <p class="text-2xl font-bold text-slate-900 mt-1"><?php echo e($mtbfData["total_failures"]); ?></p>
    </div>
    <div class="bg-white rounded-xl border border-slate-200 p-4 flex flex-col justify-between">
        <p class="text-xs text-slate-400 uppercase tracking-wide font-medium">Last Failure</p>
        <div class="mt-1"><?php if($mtbfData["last_failure"]): ?><p class="text-sm font-semibold text-slate-800"><?php echo e($mtbfData["last_failure"]); ?></p><p class="text-xs text-slate-500 mt-0.5">(<?php echo e($mtbfData["days_since"]); ?> hari lalu)</p><?php else: ?><p class="text-sm font-semibold text-slate-800">—</p><?php endif; ?></div>
    </div>
</div>
        ...
        <div class="flex items-center gap-3">
            <form method="GET" class="flex items-center gap-2">
                ...
                <select name="range" id="yearFilter"
                    ...
                    <option value="all" <?php echo e($range === "all" ? "selected" : ""); ?>>Semua</option>
                    ...
                    <option value="<?php echo e($year); ?>" <?php echo e($range === (string) $year ? "selected" : ""); ?>><?php echo e($year); ?></option>
...
<div class="rounded-xl border border-amber-300 bg-amber-50 p-5 mb-6">
    <div class="flex items-start gap-3">
        ...
        <div class="flex-1 min-w-0">
            ...
            <p class="text-sm text-amber-800 mt-1"><?php echo e($vibrationInsight['message']); ?></p>
            <div class="flex flex-wrap gap-2 mt-3">
                <a href="<?php echo e(route('cm.findings')); ?>?equipment_tag=<?php echo e(urlencode($equipment->equipment_tag)); ?>"
            ...
            <?php if($vibrationInsight['projected_days_to_danger']): ?>
                <p class="text-xs text-amber-600 mt-2">
                    <span class="font-medium">Rekomendasi:</span> jadwalkan PdM inspection / tindak lanjut
                    sebelum ~<?php echo e($vibrationInsight['projected_days_to_danger']); ?> hari ke depan (proyeksi menyentuh batas Danger).
                </p>
            <?php elseif($vibrationInsight['projected_days_to_alarm']): ?>
                <p class="text-xs text-amber-600 mt-2">
                    <span class="font-medium">Rekomendasi:</span> jadwalkan PdM inspection / tindak lanjut
                    sebelum ~<?php echo e($vibrationInsight['projected_days_to_alarm']); ?> hari ke depan (proyeksi menyentuh batas Alarm).
                </p>
            <?php else: ?>
                <p class="text-xs text-amber-600 mt-2">
...
<div class="bg-white rounded-xl border border-slate-200 overflow-hidden mb-6">
            ...
            <tbody class="divide-y divide-slate-50">
                <?php $__empty_1 = true; $__currentLoopData = $readings; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $r): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    ...
                    <tr class="hover:bg-slate-50 transition-colors">
                        <td class="px-4 py-3 text-xs text-slate-600 whitespace-nowrap"><?php echo e(\Carbon\Carbon::parse($r->tanggal)->format("d M Y")); ?></td>
                        <td class="px-4 py-3 text-center"><span class="inline-flex px-2.5 py-0.5 text-xs font-medium rounded-full <?php echo e($bcColor); ?>"><?php echo e(ucfirst(str_replace("_", " ", $rs))); ?></span></td>
                        <td class="px-4 py-3 text-center font-mono text-xs text-slate-700"><?php echo e($mvStr); ?></td>
                        <td class="px-4 py-3 text-center font-mono text-xs text-slate-700"><?php echo e($pvStr); ?></td>
                        <td class="px-4 py-3 text-center font-mono text-xs text-slate-700"><?php echo $tempStr; ?></td>
                        <td class="px-4 py-3 text-right font-mono text-xs text-slate-700"><?php echo e($r->ampere ? number_format($r->ampere, 1) : "—"); ?></td>
                        <td class="px-4 py-3 text-xs text-slate-500 max-w-[200px] truncate" title="<?php echo e($r->remark ?? ""); ?>"><?php echo e($r->remark ?? "—"); ?></td>
                    </tr>
    ...
    <?php if($readings->hasPages()): ?>
        <div class="px-5 py-3 border-t border-slate-100"><?php echo e($readings->appends(["range" => $range])->links()); ?></div>
    <?php endif; ?>
</div>
<div class="bg-white rounded-xl border border-slate-200 overflow-hidden mb-6">
    <div class="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
        <h3 class="text-sm font-semibold text-slate-900">Temuan (Findings)</h3>
        <span class="text-xs text-slate-400"><?php echo e($equipment->findings->count()); ?> total</span>
    </div>
    <div class="overflow-x-auto">
        <table class="w-full text-sm">
            <thead>
            ...
            <tbody class="divide-y divide-slate-50">
                <?php $__empty_1 = true; $__currentLoopData = $equipment->findings->sortByDesc('tanggal_temuan'); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $f): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    <?php
                    ...
                    ?>
                    <tr class="hover:bg-slate-50 transition-colors">
                        <td class="px-4 py-3"><a href="<?php echo e(route('cm.findings.show', $f->kode_finding)); ?>" class="font-mono text-xs text-teal-700 font-medium hover:text-teal-900 hover:underline transition-colors"><?php echo e($f->kode_finding); ?></a></td>
                        <td class="px-4 py-3 text-center"><span class="inline-flex px-2.5 py-0.5 text-xs font-medium rounded-full <?php echo e($sevColor); ?>"><?php echo e(ucfirst($f->severity)); ?></span></td>
                        <td class="px-4 py-3 text-xs text-slate-600"><?php echo e($f->kategori ?? "—"); ?></td>
                        <td class="px-4 py-3 text-xs text-slate-700 max-w-[250px] truncate" title="<?php echo e($f->deskripsi); ?>"><?php echo e($f->deskripsi ?? "—"); ?></td>
                        <td class="px-4 py-3 text-center"><?php if($isOpen): ?><span class="inline-flex items-center px-2.5 py-0.5 text-xs font-medium rounded-full bg-amber-100 text-amber-700"><span class="w-1.5 h-1.5 rounded-full bg-amber-500 mr-1.5"></span>Open</span><?php else: ?><span class="inline-flex items-center px-2.5 py-0.5 text-xs font-medium rounded-full bg-green-100 text-green-700"><span class="w-1.5 h-1.5 rounded-full bg-green-500 mr-1.5"></span>Closed</span><?php endif; ?></td>
                        <td class="px-4 py-3 text-xs text-slate-600"><?php echo e($f->pic ?? "—"); ?></td>
                        <td class="px-4 py-3 text-xs text-slate-500 whitespace-nowrap"><?php echo e($f->tanggal_temuan ? \Carbon\Carbon::parse($f->tanggal_temuan)->format("d M Y") : "—"); ?></td>
                        <td class="px-4 py-3 text-right text-xs <?php echo e($isOpen && $f->hari_open > 7 ? "text-red-600 font-semibold" : "text-slate-600"); ?>"><?php echo e($f->hari_open ?? "—"); ?></td>
                    </tr>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
...
<div id="editSpekModal" class="fixed inset-0 z-50 hidden bg-black/40 flex items-center justify-center p-4" onclick="if(event.target===this)this.classList.add('hidden')">
    <div class="bg-white rounded-xl shadow-xl w-full max-w-md max-h-[90vh] overflow-y-auto">
        ...
        <form method="POST" action="<?php echo e(route('cm.equipment-update', $equipment->equipment_tag)); ?>" class="p-6 space-y-4">
            <?php echo csrf_field(); ?>
            <?php echo method_field('PUT'); ?>
            <div>
                <label class="block text-xs font-medium text-slate-600 mb-1">Tipe Lubrikasi</label>
                <input type="text" name="tipe_lubrikasi" value="<?php echo e(old('tipe_lubrikasi', $equipment->tipe_lubrikasi)); ?>"
            ...
            <div>
                ...
                <input type="text" name="manufacturer" value="<?php echo e(old('manufacturer', $equipment->asset?->manufacturer)); ?>"
                       class="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-teal-400 focus:border-teal-400"
                       placeholder="—">
            </div>
            <div>
                <label class="block text-xs font-medium text-slate-600 mb-1">Model Number</label>
                <input type="text" name="model_number" value="<?php echo e(old('model_number', $equipment->asset?->model_number)); ?>"
                       class="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-teal-400 focus:border-teal-400"
                       placeholder="—">
            </div>
            <div>
                <label class="block text-xs font-medium text-slate-600 mb-1">Construct Year</label>
                <input type="number" name="construct_year" value="<?php echo e(old('construct_year', $equipment->asset?->construct_year)); ?>"
                       class="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-teal-400 focus:border-teal-400"
...
<?php $__env->stopPush(); ?>
<?php echo $__env->make("layouts.app", array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\ASUS\oleochemicalReport\resources\views/cm/equipment-show.blade.php ENDPATH**/ ?>